...
public class BottomBar extends LinearLayout implements View.OnClickListener, View.OnLongClickListener {
    ...
    private void addViewsToBottomBar(BottomBarTab[] tabsToAdd) {
    ...
    public void resizeTabsToCorrectSizes() {
        List<Integer> visibleViewPos = new ArrayList<>();
        ...
        LinearLayout containerView = (LinearLayout)findViewById(R.id.bb_bottom_bar_item_container);
        for (int i = 0; i <= containerView.getChildCount(); i++) {
            if (containerView.getChildAt(i) instanceof BottomBarTab
                    && (containerView.getChildAt(i)).getVisibility() == View.VISIBLE) {
        ...
        int proposedItemWidth = Math.min(
                MiscUtils.dpToPixel(getContext(), viewWidth / visibleViewPos.size()),
                maxFixedItemWidth
        );

        inActiveShiftingItemWidth = (int) (proposedItemWidth * 0.9);
        activeShiftingItemWidth = (int) (proposedItemWidth + (proposedItemWidth * ((visibleViewPos.size() - 1) * 0.1)));
        int height = Math.round(getContext().getResources()
                .getDimension(R.dimen.bb_height));

        for (Integer i: visibleViewPos) {
            BottomBarTab view = (BottomBarTab)containerView.getChildAt(i);
